...
(function () {
  ...
  // Allow Tab between number inputs
  ...
  el.tripleC.addEventListener("keydown", (e) => {
    if (e.key === "Enter") { e.preventDefault(); submitTriple(); }
  });
  ...
  function submitTriple() {
    ...
    el.tripleSubmitBtn.disabled = true;
    socket.emit("propose_triple", { a, b, c });
  ...
  socket.on("triple_invalid", ({ message }) => {
    el.tripleSubmitBtn.disabled = false;
